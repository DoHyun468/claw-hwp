...
> - **Font (`font_family`) on the in-place `apply_text_style` raw-patch path** → Hancom Docs ✓. `applyTextStyleInPlace` registers the requested font in DocInfo's FACE_NAME table (appended to all 7 language slots, with the per-slot `HWPTAG_ID_MAPPINGS` counts bumped) and writes the 7 resolved face-ids into the new CHAR_SHAPE — so the run renders in that font. **No whitelist — any font name is registered.** Whether it *renders* depends on the viewer: the Hancom desktop app has the full font library; **Hancom Docs (web) renders a smaller fixed set** and substitutes the default face for the rest (same as Hancom's own editor). Parity with the `.hwpx` track's 91-font set. Web-render-confirmed names (A-set, 2026-06): Korean — `함초롬바탕`, `맑은 고딕`, `해피니스 산스 볼드/레귤러/타이틀/VF`, `Pretendard` (Thin/ExtraLight/Light/Medium/(regular)/SemiBold/Bold/ExtraBold/Black), `Apple SD 산돌고딕 Neo`, `HY견고딕`, `HY견명조`, `HY그래픽`, `HY헤드라인M`, `SpoqaHanSans`, `Cafe24 Ssurround Bold`, `카페24 슈퍼매직`; Latin — `Arial`, `Calibri`, `Comic Sans MS`, `Courier New`, `Georgia`, `Impact`, `Plantagenet Cherokee`, `Tahoma`, `Times New Roman`, `Trebuchet MS`, `Verdana`, `Symbol`. **Avoid `Webdings` / `Wingdings` / `Wingdings 2` / `Wingdings 3` for normal text** — they are dingbat fonts with no letterforms, so in Hancom web every character (including the label) renders as unreadable symbol glyphs (looks broken). They only make sense for intentional symbol glyphs. Desktop-app-only candidates (B-set) include 시스템/고전 (`바탕`/`돋움`/`굴림`/`궁서`…), `나눔*`, more `HY*`/`휴먼*`/`양재*`/`MD*`/`한컴 *` faces — use an exact name. (Verified: A-set renders in Hancom Docs identically to Hancom's own font apply.)
...
| `insert_page_number` | — | `align` | Puts an auto-incrementing **page number in the footer** of every page. `align` = `"center"` (default) / `"left"` / `"right"`. Alignment references an existing paragraph shape with that alignment (real documents have them); if the document has none, the number follows the default paragraph alignment. No anchor or text needed. Section 0 only (no DocInfo change). Hancom-Docs render verified (centered on documents that have a centered paragraph shape). |
