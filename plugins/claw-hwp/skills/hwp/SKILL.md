...
| `insert_shape` | — | `anchor`, `cell` | Inserts a 도형 drawing object — `shape` = `"rect"` (default) / `"ellipse"` / `"line"` / `"arc"` — as a floating object near `anchor` text (or the first body paragraph). **`cell: {row, col, para?, control?}`** instead drops the shape **inside that table cell** as a treat-as-char paragraph, **centered** by default (the cell's row grows to fit). Default size ~53×24 mm. Hancom-Docs render verified (body + centered in-cell). |
| `insert_textbox` | `text` | `anchor` | Inserts a 글상자 / text box (a rect drawing object carrying `text`) as a floating object near `anchor` (or the first body paragraph). The geometry is GT-verbatim (default-text box is byte-identical to Hancom's); only the inner paragraph varies with `text`. Default size ~53×24 mm. Section 0 only (no DocInfo change). Hancom-Docs render verified. |
...
| `insert_image` | `path` | `anchor`, `cell` | Inserts a 그림/image (`path` = a local image file ≤ ~4 KB deflated) as a new paragraph near `anchor` (or after the last body paragraph). **`cell: {row, col, para?, control?}`** instead drops the image **inside that table cell** as a centered treat-as-char paragraph (the cell's row grows to fit; `para`/`control` address the table, default 0/0). **Hancom-Docs compatible raw-patch**: creates the `BinData` storage folder + stream, the DocInfo BinData def, and a gso `$pic` cluster reproduced from Hancom's own output. Default size ~21×13 mm (fit-to-cell-width scaling pending — aligned with the HWPX team). Hancom-Docs render verified (body + in-cell, mini + regular-FAT). (This is the Hancom-Docs path; the older `append_image` goes through rhwp/Hancom-Office.) |
| `insert_chart` | — | `chart_type`, `anchor`, `cell`, `rows`, `cols`, `categories`, `series`, `data`, `title`, `float` | Inserts a 차트/chart as a new paragraph near `anchor` (or after the last body paragraph). **`cell: {row, col, para?, control?}`** instead drops the chart **inside that table cell**, centered as a like-char paragraph (the cell's row grows to fit). `chart_type` = `0`–`19` selects the chart kind (0 column, 1 stacked column, 2 line, 6 pie, 8 doughnut, 16 3-D pie, … — bar/line/area/radar/pie/doughnut family). **Hancom-Docs compatible raw-patch**: creates the `BinData` folder + a chart OLE stream (Hancom re-renders the chart from the OLE's embedded OOXML chart XML), the DocInfo BinData def, and a gso `ole$` cluster. **Data is editable**: `rows`/`cols` set the category/series counts (auto-labelled 항목 N / 계열 N with sample values), or pass `categories` (row labels), `series` (`[{name, values:[…]}]`), or `data` (`[[…],[…]]` = values per series) for explicit values. Placed **like-char** by default so text flows cleanly above/below it; pass `float:true` for the floating original (text wraps beside it). Clean docs only (no existing `BinData`). Hancom-Docs render verified — all 20 types + custom rows/cols (mini + regular-FAT). |
