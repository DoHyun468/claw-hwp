...
| `set_object_property` | `object_index` (default 0) + ≥1 of: `fill`, `border_color`, `border_width_mm`, `border_type`, `margins:[l,r,t,b]`/`margin_mm`, `wrap`, `fill_transparency` | — | Edits an existing **drawing object** (shape/도형) in place by patching its gso CTRL_HEADER + SHAPE_COMPONENT. **`fill`** = `"#RRGGBB"` solid fill color (면 색). **`border_color`** = `"#RRGGBB"` line/border color (선 색). **`border_width_mm`** = border thickness in mm. **`border_type`** = line style: `"solid"`/`"dotted"`/`"dashed"`/`"dash-dot"`/`"dash-dot-dot"`/`"long-dash"`/`"circle-dot"`/`"double"`. **`fill_pattern`** = hatch pattern (채우기 무늬): `"none"`/`"horizontal"`/`"vertical"`/`"down-diagonal"`/`"up-diagonal"`/`"grid"`/`"cross"`, with optional **`fill_pattern_color`** (`"#RRGGBB"`). **`fill_transparency`** = fill transparency 0-100 (% — 0 opaque, alpha = round(t×255/100)). **`margins:[left,right,top,bottom]`**/**`margin_mm`** = the object's outer margin (개체↔글 간격) in mm. **`wrap`** = text placement (본문과의 배치): `"inline"` (글자처럼 취급), `"square"` (어울림), `"topbottom"`, `"behind"` (글 뒤로), `"front"` (글 앞으로) — same encoding as the table's `table_wrap`. `object_index` selects the Nth drawing object (default first). Byte-identical to Hancom's own `object-prop` output; Section 0 only (no DocInfo change). Hancom-Docs render verified. |
...
| `insert_shape` | — | `shape`, `anchor` | Inserts a 도형 drawing object — `shape` = `"rect"` (default) / `"ellipse"` / `"line"` / `"arc"` — as a floating object near `anchor` text (or the first body paragraph). Default size ~53×24 mm. Section 0 only (no DocInfo change). Hancom-Docs render verified. |
