...
> - **Font (`font_family`) on the in-place `apply_text_style` raw-patch path** → Hancom Docs ✓. `applyTextStyleInPlace` registers the requested font in DocInfo's FACE_NAME table (appended to all 7 language slots, with the per-slot `HWPTAG_ID_MAPPINGS` counts bumped) and writes the 7 resolved face-ids into the new CHAR_SHAPE — so the run renders in that font. **Hancom Docs has a fixed supported-font set**; a font in the set renders, anything else falls back to the default face (same as Hancom's own editor). Supported names (2026-06): `함초롬바탕`, `맑은 고딕`, `해피니스 산스`, `Pretendard` (Black/Bold/ExtraBold/ExtraLight/Light/Medium/SemiBold/Thin), `Apple SD 산돌고딕 Neo`, `HY견고딕`, `HY견명조`, `HY그래픽`, `HY헤드라인M`, `SpoqaHanSans`, `Cafe24 Ssurround`, `카페24 슈퍼매직`. Use an exact name from this set. (Verified: `HY헤드라인M` renders identically to Hancom's own font apply.)
