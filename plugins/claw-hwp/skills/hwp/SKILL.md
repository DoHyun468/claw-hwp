...
| `set_table_property` | `table_index` (default 0) + ≥1 of: `margins:[l,r,t,b]`/`margin_mm`, `page_split` | — | Sets table-level properties in place. **`margins:[left,right,top,bottom]`** (mm) or **`margin_mm`** = the table's **outer margin** (space between the table and surrounding text/page — 바깥 여백; patches the table CTRL_HEADER `" lbt"`, default 1 mm all sides). **`page_split`** = how the table divides at a page boundary (patches the TABLE record attr bits 0-1): `"none"` (don't split — table moves whole), `"cell"` (split inside cells), `"table"` (split at row boundaries — the default). `table_index` selects the Nth table in the section (default first). Byte-identical to Hancom's own `table-cell-prop` output; Section 0 only (no DocInfo change). Hancom-Docs render verified. |
