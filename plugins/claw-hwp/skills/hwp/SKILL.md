...
| `insert_shape` | — | `shape`, `anchor` | Inserts a 도형 drawing object — `shape` = `"rect"` (default) / `"ellipse"` / `"line"` — as a floating object near `anchor` text (or the first body paragraph). Default size ~53×24 mm. Section 0 only (no DocInfo change). Hancom-Docs render verified. (Arc not yet supported.) |
