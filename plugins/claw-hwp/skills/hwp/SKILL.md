...
| `delete_object` | `index` | — | **Deletes a floating object** (그림/image, 차트/chart, 도형/shape) entirely. `index` = the 0-based ordinal of the object in document order (same order `extract_text --inspect` / a find-objects sweep reports). Removes the object's gso paragraph from Section 0; for image/chart objects it also removes the `BinData` stream (bytes zeroed + chain freed) and HWPTAG_BIN_DATA def, then **renumbers every higher storage id down by 1** (BIN_DATA defs + ID_MAPPINGS count + each remaining gso's binDataID, and each higher `BIN000N` stream is **renamed** to close the gap — keeping its own bytes/extension in place) — Hancom resolves a binDataID by position, so a gap renders the *higher* objects as broken-image boxes; renumbering keeps them intact. Two GT-faithful edge cases are handled: if the deleted object's paragraph was the section's **last** paragraph, a terminal empty paragraph is kept (else Hancom rejects the file); if the **last/only** binary is deleted, the now-empty `BinData` folder is removed entirely (a clean no-binary `.hwp` has none). Shapes (no `BinData`) just drop the gso. Multiple deletes are sorted **descending** so an earlier delete never shifts a later target. GT (한컴독스 delete) + Hancom-Docs render verified across image (multi / last), chart (with & without other binaries), and lone-delete; mini + regular-FAT. |
