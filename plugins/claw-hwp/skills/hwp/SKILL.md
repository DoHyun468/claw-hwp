...
| `set_cell_property` | `section`, `para`, `control`, `row`, `col` + ≥1 of: `valign`, `height_mm`, `width_mm`, `margin_mm`/`margins` | — | Sets one table cell's properties by patching its LIST_HEADER (no DocInfo change). `valign` = `"top"`/`"middle"`/`"bottom"` (vertical text alignment); `height_mm`/`width_mm` = cell size in mm; `margin_mm` = all inner margins, or `margins:[l,r,t,b]`. Setting one cell's `height_mm` makes its whole row that tall (row renders at max). Section 0 only. Hancom-Docs render verified. |
| `set_table_property` | `table_index` (default 0) + `margins:[l,r,t,b]` or `margin_mm` | — | Sets a table's **outer margin** (the space between the table and surrounding text/page — 바깥 여백) by patching the table's CTRL_HEADER (`" lbt"`) in place. `margins:[left,right,top,bottom]` in mm, or `margin_mm` for all four. `table_index` selects the Nth table in the section (default first). Default is 1 mm all sides. Byte-identical to Hancom's own `table-cell-prop --table-margin` output; Section 0 only (no DocInfo change). Hancom-Docs render verified. |
