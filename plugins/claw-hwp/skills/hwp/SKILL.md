...
| `delete_object` | `index` | — | **Deletes a floating object** (그림/image, 차트/chart, 도형/shape) entirely. `index` = the 0-based ordinal of the object in document order (same order `extract_text --inspect` / a find-objects sweep reports). Removes the object's gso paragraph from Section 0; for image/chart objects it also removes the `BinData` stream's bytes (zeroed) and HWPTAG_BIN_DATA def, then **renumbers every higher storage id down by 1** (BIN_DATA defs + ID_MAPPINGS count + each remaining gso's binDataID + the BIN000N chain pointers) — Hancom resolves a binDataID by position, so a gap renders the *higher* objects as broken-image boxes; renumbering keeps them intact (GT: 한컴독스 delete renumbers; render-verified deleting a middle image leaves the others clean). Shapes (no `BinData`) just drop the gso. To delete several at once, the dispatcher sorts indices **descending** so an earlier delete never shifts a later target. Hancom-Docs render verified (mini + regular-FAT). |
