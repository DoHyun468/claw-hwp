...
| `insert_image` | `path` | `anchor`, `cell` | Inserts a 그림/image (`path` = a local image file ≤ ~4 KB deflated) as a new paragraph near `anchor` (or after the last body paragraph). **`cell: {row, col, para?, control?}`** instead drops the image **inside that table cell** as a treat-as-char paragraph (the cell's row grows to fit; `para`/`control` address the table, default 0/0). **Hancom-Docs compatible raw-patch**: creates the `BinData` storage folder + stream, the DocInfo BinData def, and a gso `$pic` cluster reproduced from Hancom's own output. Default size ~21×13 mm (fit-to-cell-width scaling pending — aligned with the HWPX team). Hancom-Docs render verified (body + in-cell, mini + regular-FAT). (This is the Hancom-Docs path; the older `append_image` goes through rhwp/Hancom-Office.) |
