...
| `apply_paragraph_style` | `index`, + any of `align` ("LEFT"/"CENTER"/"RIGHT"/"JUSTIFY"/"DISTRIBUTE"), `indent` (HWP units), `lineSpacing` (percent, e.g. 160) | Clones `paraPr[0]`, mutates, retargets paragraph `index`. Bumps `hh:paraProperties@itemCnt`. **⚠️ Hancom Docs web limitation:** this creates a *new* paraPr, and Hancom web's loader normalizes synthesized paraPrs — it reverts the align/indent/spacing and can add spurious numbering (`heading` NONE→NUMBER). So the formatting holds in Hancom Office desktop / the preview renderer but is **stripped on Hancom Docs web** (verified by round-trip). For web-safe alignment, set it at creation time (`create.js`) or use a single-cell table (`para_line`) for banded/aligned blocks. |
