...
| `delete_object` | `target` (`image`/`chart`/`shape`/`equation`), `index` | — | **Deletes a floating object.** Removes its enclosing `<hp:p>` (no empty line left) + drops its external part & manifest item (image→`BinData/`, chart→`Chart/`). shape/textbox(=rect)/equation have no external part → only the paragraph goes. `index` is 0-based in document order (same addressing as `set_object_*`). **NOTE:** remaining parts keep their numbers (a gap, e.g. `image1`+`image3`) — refs are by id so it renders identically on Hancom and the next insert fills the gap (gap-safe). Hancom itself renumbers contiguous on delete (`delete-obj-*` GT); we don't, because it's render-equivalent and lower-risk. (`delete_image` is the older by-name variant — leaves an empty paragraph; prefer `delete_object` for charts/shapes/clean removal.) |
