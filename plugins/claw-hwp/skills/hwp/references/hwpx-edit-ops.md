...
| `apply_paragraph_style` | `index`, + any of `align` ("LEFT"/"CENTER"/"RIGHT"/"JUSTIFY"/"DISTRIBUTE"), `indent` (HWP units), `lineSpacing` (percent, e.g. 160) | Sets paragraph properties on paragraph `index`. **`align` is Hancom-web-safe** (`webSafe: true` in the result): instead of synthesizing a paraPr — which Hancom web normalizes away (align reverts + spurious numbering) — it reuses an existing clean native paraPr with that alignment, or injects one built from the Hancom-native stub (heading NONE, that align) + the `hwpunitchar` namespace, then retargets the paragraph. Verified by Hancom-web render. **⚠️ `indent` / `lineSpacing` still use the clone path** (clone `paraPr[0]`, mutate, bump `hh:paraProperties@itemCnt`) and are **stripped by Hancom Docs web** (hold in Hancom Office desktop / preview only); set those at creation time (`create.js`) for web fidelity. Combining `align` with `indent`/`lineSpacing` routes through the clone path (so the align loses its web-safety) — apply alignment in its own call to keep it web-safe. |
