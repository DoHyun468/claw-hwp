...
| `type` | Args | Notes |
|--------|------|-------|
| `set_bullet_list` | `index`, `char?` (e.g. `"▶"`, `"◯"`, `"□"`, `"★"`, `"■"`, `"◆"`, `"✓"`), `level?` (default `0`) | Marks paragraph `index` as a bullet item. **Hancom Docs web behavior:** Hancom's web viewer silently strips `<hh:heading type="BULLET">` from any paraPr it judges as foreign — including ones whose XML is byte-identical to Hancom-native paraPrs. The op handles this with a three-step strategy: (1) if the host doc already carries a Hancom-authored BULLET paraPr (round-tripped through Hancom Office / Hancom Docs), retarget the paragraph's `paraPrIDRef` to it (`reusedHancomNative: true`). (2) Otherwise inject a Hancom-round-trip stub (Scripts/, xmlns:hwpunitchar, native BULLET/NUMBER paraPrs from `scripts/templates/hancom_native_stub.hwpx`) into the doc and retry the reuse — works for any plain hwpx. (3) If the stub file is missing, fall back to prepending the bullet char + a space to the paragraph text as literal prefix (`fallback: "text-prefix"`). Both the reuse and inject paths produce real Hancom-rendered ▶ from the paragraph's paraPr; the text-prefix path is visual-only but works everywhere. |
| `set_number_list` | `index`, `level?` (default `0`), `style?` (`"korean"` or `"decimal"`), `number?` (used only by the text-prefix fallback) | Marks paragraph `index` as a numbered item. With `style: "korean"`, registers a numbering whose levels cycle `1.` / `가.` / `1)` / `가)` / `(1)` / `(가)`. With `style: "decimal"`, registers one whose levels are `1.` / `1.1.` / `1.1.1.` / …. Without `style`, uses the doc's existing numbering id=1 (visual format depends on the template). `level` 0–5 picks the format on that numbering's level chain. **Hancom Docs web behavior:** same three-step strategy as `set_bullet_list` — reuse → stub inject → text-prefix fallback. The text-prefix path prepends `${number ?? 1}. ` (default `"1. "`); the caller passes `number` to keep the sequence consistent across multiple paragraphs since this op doesn't auto-increment. The reuse and inject paths render real `1./2./...` from `<hh:numbering>` and ignore `number`. |
